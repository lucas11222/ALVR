...
override PASSTHROUGH_BRIGHTNESS: f32 = 0.4;
...
fn fragment_main(@location(0) uv: vec2f) -> @location(0) vec4f {
    ...
    var color: vec3f = textureSample(stream_texture, stream_sampler, corrected_uv).rgb;
    ...
    return vec4f(color, 1.0 - PASSTHROUGH_BRIGHTNESS);
}
